...
| `0016_gate8_reconstruction_support` | implemented measured dual-time reconstruction index only; replaces the proposed `0016_gate7_reconstruction_support` placeholder | Slice-G read composition only; no persisted management position, duplicate Decision snapshot, backfill, or summary truth |
